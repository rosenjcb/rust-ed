...
use crossterm::{cursor, style, terminal, Color, Colorize, Terminal};
...
impl Screen {
    ...
    pub fn delete(&mut self) {
        ...
        self.buffer[pos.1 as usize]
            .inner
            .insert(pos.0 as usize, ' ');
        self.terminal.write(" ");
        cursor.move_left(1);
    }
        ...
        match dir {
            Direction::Right => {
                ...
                if self.highlight.len() == 0
                    || self.highlight.back().expect("No coord found") == prev_coord
                {
            ...
            Direction::Left => {
                ...
                if self.highlight.len() == 0
                    || self.highlight.front().expect("No coord found") == prev_coord
                {
    ...
    pub fn cancel_highlight(&mut self, coord: &(u16, u16)) {
        ...
        let s: String = self
            .highlight
            .iter()
            .map(|selection| self.get_char(selection))
            .collect();
